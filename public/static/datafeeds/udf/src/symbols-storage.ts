...
function extractField<Field extends keyof ExchangeDataResponseSymbolData>(
  data: ExchangeDataResponse,
  field: Field,
  arrayIndex: number = 0, // Default to 0 if not provided
  valueIsArray: boolean = false
): ExchangeDataResponseSymbolData[Field] | undefined {
  const value = data[field];

  if (Array.isArray(value) && valueIsArray) {
    // If the value is an array and expected to be an array, return it directly
    return value as ExchangeDataResponseSymbolData[Field];
  } else if (Array.isArray(value) && !valueIsArray) {
    // If the value is an array but not expected to be an array, attempt to return an element
    return value[arrayIndex] as ExchangeDataResponseSymbolData[Field];
  } else {
    // For non-array values or if the value is missing
    return value as ExchangeDataResponseSymbolData[Field];
  }
}
